...
final eatenMealsProvider =
    StreamProvider.family<List<EatenMealModel>, String>((ref, petId) {
...
class EatenMealService {
  ...
  Stream<List<EatenMealModel>> getEatenMealsStream(String petId) {
    return _firestore
        ...
        .map((snapshot) => snapshot.docs
            .map((doc) => EatenMealModel.fromDocument(doc))
            .toList());
  }

  Future<void> addEatenMeal(String petId, EatenMealModel meal) async {
    await _firestore
        .collection('app_users')
        .doc(FirebaseAuth.instance.currentUser!.uid)
        .collection('pets')
