...
/// Screen for managing behaviorist reminders.
class BehavioristReminderScreen extends ConsumerStatefulWidget {
  const BehavioristReminderScreen({super.key});

  @override
  ConsumerState<BehavioristReminderScreen> createState() =>
      _BehavioristReminderScreenState();
}

class _BehavioristReminderScreenState
    extends ConsumerState<BehavioristReminderScreen> {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        title: Text(
          'B E H A V I O R I S T  R E M I N D E R S',
          style: TextStyle(
      ...
      body: Column(
        children: [
          _buildToggleButtons(),
          Expanded(
            child: StreamBuilder<List<BehavioristReminderModel>>(
              stream: ref
                  .read(behavioristReminderServiceProvider)
                  .getBehavioristReminders(userId),
              builder: (context, snapshot) {
                ...
                if (filteredReminders.isEmpty) {
                  return Center(
                    child: Text(
                      isCurrentReminders
                          ? 'No upcoming behaviorist appointments.'
                          : 'No behaviorist appointment history.',
                      style: TextStyle(
                        color: Theme.of(context).primaryColorDark,
                      ),
                    ),
  ...
  List<BehavioristReminderModel> _filterReminders(
      List<BehavioristReminderModel> reminders, DateTime now) {
  ...
  Widget _buildReminderCard(BehavioristReminderModel reminder) {
    return Card(
      color: Theme.of(context).colorScheme.primary,
    ...
    if (reminderDateTime.isAfter(DateTime.now())) {
      ...
      for (var notification in _additionalNotifications) {
        ...
        if (earlyNotificationTime.isAfter(DateTime.now())) {
          final earlyNotificationId = mainNotificationId + value;
          earlyNotificationIds.add(earlyNotificationId);
          await NotificationService().createSingleNotification(
            id: earlyNotificationId,
            title: 'Early Behaviorist Reminder',
            body: _generateNotificationBody(reminder),
            dateTime: earlyNotificationTime,
            payload: 'behaviorist_reminder_early',
          );
        }
      }
  ...
  String _generateNotificationBody(BehavioristReminderModel reminder) {
    ...
    return 'Behaviorist reminder for: $selectedPets - ${reminder.reason}';
  }
